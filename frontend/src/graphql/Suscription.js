...
export const reporteAireAcondicionado = gql `
  subscription aire_acondicionado_aggregate(
    $fechainicio: date
    $fechafin: date
  ) {
    aire_acondicionado_aggregate(
      where: { fecha: { _gte: $fechainicio, _lte: $fechafin } }
    ) {
      aggregate {
        count
      }
...
export const reporteEdificios = gql `
  subscription detalle_mantenimiento_edificios_aggregate(
    $fechainicio: date
    $fechafin: date
  ) {
    detalle_mantenimiento_edificios_aggregate(
      where: {
        mantenimiento_edificio: {
          fecha: { _gte: $fechainicio, _lte: $fechafin }
        }
...
export const repuestosACero = gql `
  subscription repuesto_a_cero {
    repuestos(where:{cantidad:{_eq:0}}){
      nombre
      proveedor_de_repuesto{
        nombre_proveedor
        telefono_empresa
