...
export const deleteRefrendaLicencia = gql`
  mutation delete_refrenda_documentos_motorista_by_pk($id: uuid!) {
    delete_refrenda_documentos_motorista_by_pk(id: $id) {
      id
    }
  }
`;
...
export const updateKilometrajeGlobalExistente = gql`
  mutation update_kilometraje_global(
    $id_unidad_transporte: uuid
    $kilometraje: float8
  ) {
    update_kilometraje_global(
      where: { id_unidad_transporte: { _eq: $id_unidad_transporte } }
      _inc: { kilometraje: $kilometraje }
    ) {
      affected_rows
    }
  }
`;
export const submitNuevoKilometrajeGlobal = gql`
  mutation insert_kilometraje_global_one(
    $id_unidad_transporte: uuid
    $kilometraje: float8
  ) {
    insert_kilometraje_global_one(
      object: {
        id_unidad_transporte: $id_unidad_transporte
        kilometraje: $kilometraje
      }
...
export const updateImageofEmergenciaEdificioById = gql`
  mutation update_registro_emergencias_edificios_by_pk(
    $id: uuid!
    $imagenes: String
  ) {
    update_registro_emergencias_edificios_by_pk(
      _set: { imagenes: $imagenes }
      pk_columns: { id: $id }
    ) {
      id
    }
...
export const updateRepuestotoFalse = gql`
  mutation update_repuestos_by_pk($id: uuid!) {
    update_repuestos_by_pk(_set: { activo: false }, pk_columns: { id: $id }) {
      id
    }
  }
`;
export const updateRepuestoTrue = gql`
  mutation update_repuestos_by_pk($id: uuid!) {
    update_repuestos_by_pk(_set: { activo: true }, pk_columns: { id: $id }) {
      id
    }
  }
`;
