...
import { updateMantenimientoEdificio } from "../../../../graphql/Mutations";
import { listenMantenimientoById } from "../../../../graphql/Suscription";
...
function EditarMantenimiento() {
  ...
  //MUTATION
  const [setMantenimiento] = useMutation(updateMantenimientoEdificio);

  const { data, loading, error } = useSubscription(listenMantenimientoById, {
    variables: {
      id: idMantenimiento,
    },
  });
  ...
  useEffect(() => {
    let mantenimiento = {};
    mantenimiento =
      data === undefined ? {} : data.mantenimiento_edificios_by_pk;
    setNuevoMantenimiento(mantenimiento);
  }, [data]);
  ...
  const submitMantenimiento = () => {
    ...
    setMantenimiento({
      variables: {
        ...
        id_edificio: NuevoMantenimiento.id_edificio,
        id_empleado: NuevoMantenimiento.id_empleado,
        imagen_antes: NuevoMantenimiento.imagen_antes,
        imagen_despues: NuevoMantenimiento.imagen_despues,
      },
    })
      .then((res) => {
        if (res.data) {
          ...
          <div className="grid-form-transporte">
            ...
            <div>
              <ImageSelected
                ...
                tiempo="de antes"
                ImagePreviousSelected={NuevoMantenimiento.imagen_antes}
              />
              <br />
              <br />
              <ImageSelected
                ...
                tiempo="después"
                ImagePreviousSelected={NuevoMantenimiento.imagen_despues}
              />
            </div>
